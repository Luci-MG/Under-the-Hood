...
**Intrinsic Lock:**
Each **Java object has an intrinsic lock** (also called **monitor lock**) associated with it, The thread that enters the synchronized block **acquires the intrinsic lock**. When it leaves the block, it **releases the lock**, allowing other threads to acquire it.
...
???+ example "Example with Instance-Level Locking"
    ```java
    class Counter {
        private int count = 0;
...
???+ example "Example with Class-Level Locking"
    ```java
    class Counter {
...
???+ example "Example: Synchronized Block with Fine-Grained Control"
    ```java
